...
from jembe_auth_demo.db import exceptions as exception


LOG = logging.getLogger(__name__)


_registry = collections.defaultdict(  # type:ignore
...
def _default_dupe_key_error(integrity_error, match, engine_name, is_disconnect):
    ...
    raise exception.DBDuplicateEntry(columns, value) from integrity_error
...
def _sqlite_dupe_key_error(integrity_error, match, engine_name, is_disconnect):
    ...
    raise exception.DBDuplicateEntry(columns) from integrity_error
...
def _db2_dupe_key_error(integrity_error, match, engine_name, is_disconnect):
    ...
    raise exception.DBDuplicateEntry([]) from integrity_error
...
def _raise_for_unicode_encode(error, match, engine_name, is_disconnect):
    raise exception.DBInvalidUnicodeParameter() from error
